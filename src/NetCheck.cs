...
namespace MeiQNetCheck
{
    class NetCheck : IDisposable
    {
            ...
            if (_dnsConfigCheck.checkNDSconfig())
            {
                ...
                if (_localNetCheck.checkLocalNetStat())
                {
                    ...
                    if (_pingServerNetCheck.checkNetPing(_strDomainName))
                    {
                        ...
                        _message += "[" + System.DateTime.UtcNow.AddHours(8) + "]" + "（如果您仍然存在页面不正常的情况，请尝试清理浏览器缓存及cookie）"
        ...
        public void getDNSMessageCheckToMail()
        {
            ...
            _mailMessage += "[" + System.DateTime.UtcNow.AddHours(8) + "]" + "到美洽远程服务器检测失败信息如下" + System.Environment.NewLine;
            _mailMessage += "【系统信息：】" + System.Environment.NewLine;
            _mailMessage += _systemInfo.OSInfo;

            //DNS配置
            _mailMessage += "-------------------------------------------------------------------" + System.Environment.NewLine;
            _mailMessage += "【DNS配置信息：】" + System.Environment.NewLine;
        ...
        public void getLocalNetMessageCheckToMail()
        {
            ...
            //ping   www.qq.com
            _mailMessage += "-------------------------------------------------------------------" + System.Environment.NewLine;
            _mailMessage += "【ping 到 www.qq.com的信息：】" + System.Environment.NewLine;
        ...
        public void getRemoteServerCheckToMail()
        {
            ...
            string[] domainName = _strDomainName.Split(new char[] { ':' });
            
            for (var i = 0; i < domainName.Length; i++)
            {
                _cmdRunCommand.run("cmd.exe", "ping " + domainName[i] + " -n 2 -r 9");
                _mailMessage += _cmdRunCommand.ResultMessage;
            }
